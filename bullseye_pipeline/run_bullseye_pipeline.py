...
if __name__ == '__main__':
    ...
    p.add_argument('-s', '--scans_dir', required=True)
    p.add_argument('-w', '--work_dir', required=True)
    p.add_argument('-o', '--outputdir', required=True)
    p.add_argument('--subject', dest='subject_ids', nargs='+', required=True)
    args = p.parse_args()

    bullwf = create_bullseye_pipeline(
    ...
    bullwf.run(plugin='MultiProc', plugin_args={'n_procs': 4})
