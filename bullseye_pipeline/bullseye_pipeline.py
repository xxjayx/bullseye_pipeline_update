### This version fixes a label precision issue in the original `generate_wmparc()` function,
### where label values like `24` were stored as `23.99999999` due to float64 output.
### Modified to cast label map to `int16` before saving to ensure correct integer labeling.
### Original pipeline: https://github.com/gsanroma/bullseye_pipeline/tree/master/bullseye_pipeline
...
def create_bullseye_pipeline(scans_dir, work_dir, outputdir, subject_ids, name='bullseye_pipeline'):
    ...
    bullwf.connect# These lines of code are establishing connections between nodes in the Nipype
    # workflow. Specifically, they are connecting the output of the `annot2label_rh`
    # node (which is the annotated file for the right hemisphere) to the input
    # `in_lobes_rh` of the `aparc2aseg` node. Similarly, the output of the
    # `annot2label_lh` node (annotated file for the left hemisphere) is connected to the
    # input `in_lobes_lh` of the `aparc2aseg` node.
    (annot2label_rh     , 'out_annot_file',         aparc2aseg, 'in_lobes_rh')
